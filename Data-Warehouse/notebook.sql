...
-- MAGIC %md
-- MAGIC ## DWH core-layer data display
-- MAGIC  
...
-- MAGIC %md
-- MAGIC ### Incremental load of new data into sales.order table on 2nd day
...
-- MAGIC %md
-- MAGIC ### Truncate the staging layer table from DWH to read only the new data on the 2nd day/ incremental data (CDC- Change Data Capture)
...
-- MAGIC %md
-- MAGIC ### Check whether the CDC data has been captured into "View" and "core_sales" DWH layer
-- MAGIC
...
-- By default the CDC data not captured in the core_sales, bcoz if we do a copy from "view" to "core_sales" then there is no diffrence between the "stg_sales" table and "core_sales" 
